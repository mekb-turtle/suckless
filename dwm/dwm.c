...
{
	labelmon(m, m->lt[m->sellt]->symbol);
...
void
labelmon(Monitor* m, const char* sym)
{
	Client *c;
	unsigned int n1 = 0;
//	unsigned int n2 = 0;
...
drawbar(Monitor *m)
{
	int x, w, tw = 0, stw = 0, tw2, mw, ew = 0;
...
/*
		...
		char selected = (m->tagset[m->seltags] & (1 << i)) > 0;
...
//			drw_rect(drw, x + boxs, bh-boxs*2, w-boxs*2, boxs,      // line at bottom of tag
	...
	if ((w = m->ww - tw - stw - x) > bh) {
		...
		*/
		if (n > 0) {
			tw2 = TEXTW(m->sel->name) + lrpad;
			mw = (tw2 >= w || n == 1) ? 0 : (w - tw2) / (n - 1);
			...
			for (c = m->clients; c; c = c->next) {
				if (!ISVISIBLE(c))
					continue;
				tw2 = MIN(m->sel == c ? w : mw, TEXTW(c->name));

				drw_setscheme(drw, scheme[m->sel == c ? SchemeSel : SchemeNorm]);
				if (tw2 > 0) /* trap special handling of 0 in drw_text */
...
/*void // broken with mononcle and grid
...
setlayout(const Arg *arg)
{
	...
	//strncpy(selmon->ltsymbol, selmon->lt[selmon->sellt]->symbol, sizeof selmon->ltsymbol); // arrange() does this
	//if (selmon->sel)
	arrange(selmon);
	//else
	drawbar(selmon);
